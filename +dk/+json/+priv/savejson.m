...
%
    ...
    global scalarCellItem;
    scalarCellItem=false;
    ...
    rootisarray=0;
    rootlevel=1;
    forceroot=dk.json.opt('ForceRootName',0,opt);
    ...
    if(dk.json.opt('Compact',0,opt)==1)
    ...
    jsonp=dk.json.opt('JSONP','',opt);
    if(~isempty(jsonp))
        json=sprintf('%s(%s);%s',jsonp,json,nl);
    end

    % save to a file if FileName is set, suggested by Patrick Rapin
    filename=dk.json.opt('FileName','',opt);
    if(~isempty(filename))
        if(dk.json.opt('SaveBinary',0,opt)==1)
...
function txt=cell2json(name,item,level,varargin)
    ...
    len=numel(item);
    ws=dk.json.opt('whitespaces_',struct('tab',sprintf('\t'),'newline',sprintf('\n'),'sep',sprintf(',\n')),varargin{:});
    padding0=repmat(ws.tab,1,level);
    padding2=repmat(ws.tab,1,level+1);
    nl=ws.newline;
    bracketlevel=~dk.json.opt('singletcell',1,varargin{:});
    if(len>bracketlevel)
        if(~isempty(name))
    ...
    for i=1:dim(1)
        ...
        for j=1:dim(2)
            scalarCellItem=isnumeric(item{i,j}) && isscalar(item{i,j});
...
function txt=struct2json(name,item,level,varargin)
    ...
    len=numel(item);
    forcearray= (len>1 || (dk.json.opt('SingletArray',0,varargin{:})==1 && level>0));
    ws=struct('tab',sprintf('\t'),'newline',sprintf('\n'));
    ws=dk.json.opt('whitespaces_',ws,varargin{:});
    padding0=repmat(ws.tab,1,level);
    padding2=repmat(ws.tab,1,level+1);
...
function txt=mat2json(name,item,level,varargin)
    ...
    if( length(size(item))>2 || issparse(item) || ~isreal(item) || ...
            (isempty(item) && any(size(item))) || dk.json.opt('ArrayToStruct',0,varargin{:}) )
        if(isempty(name))
        ...
        %if( ~scalarCellItem && numel(item)==1 && dk.json.opt('SingletArray',0,varargin{:})==0 && level>0 )
        if( numel(item)==1 && dk.json.opt('SingletArray',0,varargin{:})==0 && level>0 )
        ...
        if(isempty(name))
            ...
            if(numel(item)==1 && dk.json.opt('SingletArray',0,varargin{:})==0)
...
function txt=matdata2json(mat,level,varargin)
    ...
    floatformat=dk.json.opt('FloatFormat','%.10g',varargin{:});
    %if(numel(mat)>1)
    ...
    if(nargin>=2 && size(mat,1)>1 && dk.json.opt('ArrayIndent',1,varargin{:})==1)
        formatstr=[repmat(tab,1,level) formatstr];
    end

    txt=sprintf(formatstr,mat');
    txt(end-length(nl):end)=[];
    if(islogical(mat) && dk.json.opt('ParseLogical',0,varargin{:})==1)
    ...
    txt=[pre txt post];
    if(any(isinf(mat(:))))
        txt=regexprep(txt,'([-+]*)Inf',dk.json.opt('Inf','"$1_Inf_"',varargin{:}));
    end
    if(any(isnan(mat(:))))
        txt=regexprep(txt,'NaN',dk.json.opt('NaN','"_NaN_"',varargin{:}));
    end
end

%%-------------------------------------------------------------------------
function newname=checkname(name,varargin)
    isunpack=dk.json.opt('UnpackHex',1,varargin{:});
    newname=name;
    if(isempty(regexp(name,'0x([0-9a-fA-F]+)_','once')))
        return
    end
    if(isunpack)
        isoct=dk.json.opt('IsOctave',0,varargin{:});
        if(~isoct)
