function imdata( data, img, callback, scale, varargin )
%
% imdata( data, img, callback, scale=[1,1], varargin )
%
...
% JH
    ...
    % show image
    dk.ui.image( { scale(1)*img.x, scale(2)*img.y, img.z }, varargin{:} );

    % setup selection
    function selection(x,y,varargin)
        
        % find closest match
        kx = dk.math.closest( x/scale(1), data.x );
        ky = dk.math.closest( y/scale(2), data.y );
